// OneHForearmGrid.shader — 1H experiment fork of ForearmGrid with aspect-preserving content mapping.
...
Shader "Custom/OneHForearmGrid"
{
    Properties
    {
        ...
        _TransformTex  ("Cell Transform (R=scale G=rot B=aspect)", 2D) = "black" {}
        ...
        _PreviewTransform ("Placement Preview Transform (R=scale G=rot Z=aspect)", Vector) = (0, 0, 0, 0)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float2 RotateCellOffset(float2 p, float rotRad)
            {
            ...
            float ResolveContentAspect(half4 xf)
            {
                if (xf.b > 0.001)
                    return lerp(0.25, 4.0, xf.b);
                return 1.0;
            }

            float2 ContentSrcLocal(float2 p, float scale, float rotRad, float contentAspect)
            {
                float2 r = RotateCellOffset(p, rotRad);
                contentAspect = max(contentAspect, 0.0001);
                // Uniform user scale: stretch footprint so width:height on skin = contentAspect.
                float effScaleX = contentAspect >= 1.0 ? scale * contentAspect : scale;
                float effScaleY = contentAspect >= 1.0 ? scale : scale / contentAspect;
                return float2(r.x / effScaleX + 0.5, r.y / effScaleY + 0.5);
            }
            ...
            half GetEditSelectionContentAlpha(float2 grid, float2 meshUV)
            {
                ...
                float contentAspect = ResolveContentAspect(xf);
                float2 srcLocal = ContentSrcLocal(p, scale, rotRad, contentAspect);
            ...
            {
                return min((int)ceil(_MaxContentScale), MAX_COMPOSITE_RADIUS);
            }
            ...
            half4 SamplePreviewCellContent(float2 grid, float2 meshUV, int2 previewCoord)
            {
                ...
                float contentAspect = max(_PreviewTransform.z, 0.0001);
                float2 srcLocal = ContentSrcLocal(p, scale, rotRad, contentAspect);
